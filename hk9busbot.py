...
BOT_TOKEN = os.getenv('TELE_BOT_TOKEN', 'ENTER_HERE')


@run_async
def start(bot, update):
    bot.sendMessage(chat_id=update.message.chat_id, text="Hi. Send個九巴巴士No.來")
    logging.info('Start : %s by %s' %
                 (update.message.chat, update.message.from_user))
...
def search(bot, update):
    ...
    try:
        ...
        r = requests.get(
            "http://search.kmb.hk/KMBWebSite/Function/FunctionRequest.ashx?action=getroutebound&route=%s" % route)
        data = json.loads(r.text)
        bound = set()
        for d in data["data"]:
            bound.add(d["BOUND"])
        if (len(bound) == 1):
            keyboard = [[InlineKeyboardButton(OriCName, callback_data="route=%s&bound=1" % route)], [
                InlineKeyboardButton(DestCName, callback_data="route=%s&bound=1" % route)]]
        else:
            keyboard = [[InlineKeyboardButton(OriCName, callback_data="route=%s&bound=2" % route)], [
                InlineKeyboardButton(DestCName, callback_data="route=%s&bound=1" % route)]]
    ...
    try:
        ...
        if(getTime):
            ...
            r = requests.get("http://search.kmb.hk/KMBWebSite/Function/FunctionRequest.ashx?action=getstops&route=%s&bound=%s&serviceType=1" %
                             (route, bound), headers={'Connection': 'close'})

            data = json.loads(r.text.replace("\ue473", "邨"))
            OriCName = data["data"]["basicInfo"]["OriCName"]
            DestCName = data["data"]["basicInfo"]["DestCName"]
            if seq == '999':  # Last Stop
                station = data["data"]["routeStops"][int(
                    len(data["data"]["routeStops"]) - 1)]["CName"]
            else:
                station = data["data"]["routeStops"][int(seq)]["CName"]
            # get the time
            link = "http://etav3.kmb.hk/?action=geteta&lang=tc&route=%s&bound=%s&stop=%s&stop_seq=%s&servicetype=01" % (
                route, bound, bsiCode.replace("-", ""), seq)

            logging.debug('Getting ETA from "%s"' % (link))
            ...
            last_update = datetime.fromtimestamp(
                int(data["updated"]) / 1000).strftime('%H:%M:%S')
            last_reflash = datetime.now().strftime('%H:%M:%S')
            ...
            text += "\n<i>[Time now: %s]</i>" % last_reflash
            bot.editMessageText(text=text,
                                chat_id=query.message.chat_id,
                                message_id=query.message.message_id,
                                reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton(
                                    "F5", callback_data="route=%s&bound=%s&bsiCode=%s&seq=%s" % (route, bound, bsiCode, seq))]]),
                                parse_mode="HTML")
        ...
        for station in data["data"]["routeStops"]:
            stations.append([InlineKeyboardButton(station["CName"], callback_data="route=%s&bound=%s&bsiCode=%s&seq=%s" % (
                route, bound, station["BSICode"], station["Seq"]))])
...
updater = Updater(BOT_TOKEN, workers=64)
